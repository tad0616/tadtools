...
class PHPExcel_Writer_Excel5_Parser
{
    ...
    public function _advance()
    {
        ...
        $formula_length = mb_strlen($this->_formula);
        // eat up white spaces
        if ($i < $formula_length) {
    ...
    public function _match($token)
    {
        switch ($token) {
            ...
            default:
                ...
                elseif (preg_match('/"([^"]|""){0,255}"/', $token) and '"' != $this->_lookahead and (0 == mb_substr_count($token, '"') % 2)) {
                    return $token;
                }
    ...
    public function _expression()
    {
        ...
        if (preg_match('/"([^"]|""){0,255}"/', $this->_current_token)) {
            ...
            return $result;
        // If it's an error code
        } elseif (preg_match("/^#[A-Z0\/]{3,5}[!?]{1}$/", $this->_current_token) or '#N/A' == $this->_current_token) {
    ...
    public function _func()
    {
        ...
        $function = mb_strtoupper($this->_current_token);
