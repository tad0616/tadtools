...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
    ...
    private function _writeLabel($row, $col, $str, $xfIndex)
    {
        ...
        if ($strlen > $this->_xls_strmax) { // LABEL must be < 255 chars
            $str = mb_substr($str, 0, $this->_xls_strmax);
    ...
    private function _writeNote($row, $col, $note)
    {
        ...
        $this->_append($header . $data . mb_substr($note, 0, 2048));
    ...
    private function _writeWindow2()
    {
        ...
        $fPaged = 1;					 // 2
        $fPageBreakPreview = PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW === $this->_phpSheet->getSheetView()->getView();
    ...
    private function _writeSetup()
    {
        ...
        // Page orientation
        $fLandscape = (PHPExcel_Worksheet_PageSetup::ORIENTATION_LANDSCAPE == $this->_phpSheet->getPageSetup()->getOrientation()) ?
            0x0 : 0x1;
    ...
    public function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
    {
        ...
        //
        if (0 == PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, PHPExcel_Cell::stringFromColumnIndex($col_start))) {
            return;
        }
        if (0 == PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, PHPExcel_Cell::stringFromColumnIndex($col_end))) {
            return;
        }
        if (0 == PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_start + 1)) {
            return;
        }
        if (0 == PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)) {
            return;
        }
    ...
    private function _writeZoom()
    {
        ...
        if (100 == $this->_phpSheet->getSheetView()->getZoomScale()) {
            return;
        }
