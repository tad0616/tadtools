            ...
            if (count($pPHPExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
                foreach ($pPHPExcel->getSheet()->getHeaderFooter()->getImages() as $image) {
                    if (!isset($aMediaContentTypes[mb_strtolower($image->getExtension())])) {
                        $aMediaContentTypes[mb_strtolower($image->getExtension())] = $this->_getImageMimeType($image->getPath());

                        $this->_writeDefaultContentType(
                            $objWriter,
                            mb_strtolower($image->getExtension()),
                            $aMediaContentTypes[mb_strtolower($image->getExtension())]
                            );
                    }
                }
            }
