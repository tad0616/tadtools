...
class PHPExcel_Reader_Excel5 extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function listWorksheetNames($pFilename)
    {
        ...
        $this->_dataSize = mb_strlen($this->_data);
    ...
    public function load($pFilename)
    {
        ...
        foreach ($this->_sheets as $sheet) {
            ...
            if (isset($this->_loadSheetsOnly) && !in_array($sheet['name'], $this->_loadSheetsOnly, true)) {
                continue;
            }
...
//							echo 'Picture Object<br />';
            ...
            if (self::XLS_BIFF8 == $this->_version) {
                foreach ($this->_sharedFormulaParts as $cell => $baseCell) {
                    list($column, $row) = PHPExcel_Cell::coordinateFromString($cell);
                    if ((null !== $this->getReadFilter()) && $this->getReadFilter()->readCell($column, $row, $this->_phpSheet->getTitle())) {
    ...
    private function _readRecordData($data, $pos, $len)
    {
        ...
        if (self::MS_BIFF_CRYPTO_NONE == $this->_encryption || $pos < $this->_encryptionStartPos) {
            return $data;
        }

        $recordData = '';
        if (self::MS_BIFF_CRYPTO_RC4 == $this->_encryption) {
            ...
            while ($block != $endBlock) {
                ...
                $recordData .= $this->_rc4Key->RC4(mb_substr($data, 0, $step));
            ...
            $recordData .= $this->_rc4Key->RC4(mb_substr($data, 0, $len));

            // Keep track of the position of this decryptor.
            // We'll try and re-use it later if we can to speed things up
            $this->_rc4Pos = $pos + $len;
        } elseif (self::MS_BIFF_CRYPTO_XOR == $this->_encryption) {
...
//		$this->_userDefinedProperties = $ole->getUserDefinedProperties();
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x1E: // null-terminated string prepended by dword string length
                    ...
                    $value = mb_substr($this->_summaryInformation, $secOffset + 8 + $offset, $byteLength);
                    $value = PHPExcel_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
                    $value = rtrim($value);
                    break;
                case 0x40: // Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-time
                    $value = PHPExcel_Shared_OLE::OLE2LocalDate(mb_substr($this->_summaryInformation, $secOffset + 4 + $offset, 8));
                    break;
        ...
        for ($i = 0; $i < $countProperties; ++$i) {
            ...
            switch ($type) {
                ...
                case 0x1E:	//	null-terminated string prepended by dword string length
                    ...
                    $value = mb_substr($this->_documentSummaryInformation, $secOffset + 8 + $offset, $byteLength);
                    $value = PHPExcel_Shared_String::ConvertEncoding($value, 'UTF-8', $codePage);
                    $value = rtrim($value);
                    break;
                case 0x40:	//	Filetime (64-bit value representing the number of 100-nanosecond intervals since January 1, 1601)
                    // PHP-Time
                    $value = PHPExcel_Shared_OLE::OLE2LocalDate(mb_substr($this->_documentSummaryInformation, $secOffset + 4 + $offset, 8));
                    break;
                case 0x47:	//	Clipboard format
                    // not needed yet, fix later if necessary
                    break;
    ...
    private function _readNote()
    {
        ...
        $cellAddress = $this->_readBIFF8CellAddress(mb_substr($recordData, 0, 4));
        if (self::XLS_BIFF8 == $this->_version) {
...
//		echo '<br />';
    ...
    {
        $length = self::_GetInt2d($this->_data, $this->_pos + 2);
        $recordData = mb_substr($this->_data, $this->_pos + 4, $length);
        ...
        switch ($substreamType) {
            ...
            default:
                ...
                do {
                    $code = self::_GetInt2d($this->_data, $this->_pos);
                    $this->_readDefault();
                } while (self::XLS_Type_EOF != $code && $this->_pos < $this->_dataSize);
                break;
        }
    }
    ...
    private function _readFilepass()
    {
        ...
        if (!$this->_verifyPassword(
            'VelvetSweatshop',
            mb_substr($recordData, 6, 16),
            mb_substr($recordData, 22, 16),
            mb_substr($recordData, 38, 16),
            $this->_md5Ctxt
        )) {
    ...
    private function _readFont()
    {
        ...
        if (!$this->_readDataOnly) {
            ...
            if (self::XLS_BIFF8 == $this->_version) {
                $string = self::_readUnicodeStringShort(mb_substr($recordData, 14));
            } else {
                $string = $this->_readByteStringShort(mb_substr($recordData, 14));
            }
    ...
    private function _readFormat()
    {
        ...
        if (!$this->_readDataOnly) {
            ...
            if (self::XLS_BIFF8 == $this->_version) {
                $string = self::_readUnicodeStringLong(mb_substr($recordData, 2));
            } else {
                // BIFF7
                $string = $this->_readByteStringShort(mb_substr($recordData, 2));
            }
    ...
    private function _readSheet()
    {
        ...
        if (self::XLS_BIFF8 == $this->_version) {
            $string = self::_readUnicodeStringShort(mb_substr($recordData, 6));
            $rec_name = $string['value'];
        } elseif (self::XLS_BIFF7 == $this->_version) {
            $string = $this->_readByteStringShort(mb_substr($recordData, 6));
            $rec_name = $string['value'];
        }
    ...
    private function _readExternalBook()
    {
        ...
        if (mb_strlen($recordData) > 4) {
            ...
            $encodedUrlString = self::_readUnicodeStringLong(mb_substr($recordData, 2));
            $offset += $encodedUrlString['size'];

            // offset: var; size: var; list of $nm sheet names (Unicode strings, 16-bit length)
            $externalSheetNames = [];
            for ($i = 0; $i < $nm; ++$i) {
                $externalSheetNameString = self::_readUnicodeStringLong(mb_substr($recordData, $offset));
    ...
    private function _readExternName()
    {
        ...
        if (self::XLS_BIFF8 == $this->_version) {
            ...
            // offset: 6; size: var
            $nameString = self::_readUnicodeStringShort(mb_substr($recordData, 6));

            // offset: var; size: var; formula data
            $offset = 6 + $nameString['size'];
            $formula = $this->_getFormulaFromStructure(mb_substr($recordData, $offset));
    ...
    private function _readDefinedName()
    {
        ...
        if (self::XLS_BIFF8 == $this->_version) {
            ...
            $string = self::_readUnicodeString(mb_substr($recordData, 14), $nlen);
    ...
    private function _readPrintGridlines()
    {
        ...
        if (self::XLS_BIFF8 == $this->_version && !$this->_readDataOnly) {
    ...
    private function _readPageSetup()
    {
        ...
        if (!$this->_readDataOnly) {
            ...
            $marginHeader = self::_extractNumber(mb_substr($recordData, 16, 8));
            $this->_phpSheet->getPageMargins()->setHeader($marginHeader);

            // offset: 24; size: 8; footer margin (IEEE 754 floating-point value)
            $marginFooter = self::_extractNumber(mb_substr($recordData, 24, 8));
    ...
    private function _readPassword()
    {
        ...
        if (!$this->_readDataOnly) {
            ...
            $password = mb_strtoupper(dechex(self::_GetInt2d($recordData, 0))); // the hashed password
    ...
    private function _readRk()
    {
        ...
        // Read cell?
        if ((null !== $this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle())) {
    ...
    private function _readNumber()
    {
        ...
        if ((null !== $this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle())) {
            ...
            $numValue = self::_extractNumber(mb_substr($recordData, 6, 8));
    ...
    private function _readFormula()
    {
        ...
        if ((null !== $this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle())) {
            ...
            if ((0 == ord($recordData[6]))
                ...
                $value = self::_extractNumber(mb_substr($recordData, 6, 8));
            }
    ...
    private function _readLabel()
    {
        ...
        if ((null !== $this->getReadFilter()) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle())) {
            ...
            if (self::XLS_BIFF8 == $this->_version) {
                $string = self::_readUnicodeStringLong(mb_substr($recordData, 6));
                $value = $string['value'];
            } else {
                $string = $this->_readByteStringLong(mb_substr($recordData, 6));
                $value = $string['value'];
            }
            $cell = $this->_phpSheet->getCell($columnString . ($row + 1));
    ...
    private function _readObj()
    {
        ...
        if ($this->_readDataOnly || self::XLS_BIFF8 != $this->_version) {
            return;
        }
...
//		echo '<br />';
    ...
    private function _readWindow2()
    {
        ...
        if (PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_LAYOUT !== $this->_phpSheet->getSheetView()->getView()) {
    ...
    private function _readHyperLink()
    {
        ...
        if (!$this->_readDataOnly) {
            ...
            if ($hasDesc) {
                ...
                $desc = self::_encodeUTF16(mb_substr($recordData, 36, 2 * ($dl - 1)), false);
                $offset += 4 + 2 * $dl;
            }
            if ($hasFrame) {
            ...
            case 'URL':
                ...
                $url = self::_encodeUTF16(mb_substr($recordData, $offset, $us - 2), false);
            ...
            if ($hasText) {
                ...
                $text = self::_encodeUTF16(mb_substr($recordData, $offset, 2 * ($tl - 1)), false);
                $url .= $text;
            }
    ...
    private function _readDataValidation()
    {
        ...
        $offset = 4;
        $string = self::_readUnicodeStringLong(mb_substr($recordData, $offset));
        ...
        $cellRangeAddressList = $this->_readBIFF8CellRangeAddressList(mb_substr($recordData, $offset));
    ...
    private function _readRangeProtection()
    {
        ...
        if (!$this->_readDataOnly) {
            ...
            for ($i = 0; $i < $cref; ++$i) {
                try {
                    $cellRange = $this->_readBIFF8CellRangeAddressFixed(mb_substr($recordData, 27 + 8 * $i, 8));
                } catch (PHPExcel_Exception $e) {
                    return;
                }
            ...
            if ($cellRanges) {
                $this->_phpSheet->protectCells(implode(' ', $cellRanges), mb_strtoupper(dechex($wPassword)), true);
            }
        }
    }
...
//			var_dump($bcBitCount);
    ...
    private function _getFormulaFromData($formulaData, $additionalData = '', $baseCell = 'A1')
    {
        ...
        while (mb_strlen($formulaData) > 0 and $token = $this->_getNextToken($formulaData, $baseCell)) {
            $tokens[] = $token;
    ...
    private function _getNextToken($formulaData, $baseCell = 'A1')
    {
        ...
        case 0x17:	//	string
            ...
            $string = self::_readUnicodeStringShort(mb_substr($formulaData, 1));
            $size = 1 + $string['size'];
        ...
        case 0x1F:	//	number
            ...
            $name = 'tNum';
            $size = 9;
            $data = self::_extractNumber(mb_substr($formulaData, 1));
        ...
        case 0x64:
            $name = 'tRef';
            $size = 5;
            $data = $this->_readBIFF8CellAddress(mb_substr($formulaData, 1, 4));
            break;
        ...
        case 0x65:
            $name = 'tArea';
            $size = 9;
            $data = $this->_readBIFF8CellRangeAddress(mb_substr($formulaData, 1, 8));
            break;
        ...
        case 0x66:
            ...
            $size = 7 + $subSize;
            $data = $this->_getFormulaFromData(mb_substr($formulaData, 7, $subSize));
            break;
        ...
        case 0x69:
            ...
            $size = 3 + $subSize;
            $data = $this->_getFormulaFromData(mb_substr($formulaData, 3, $subSize));
            break;
        ...
        case 0x6C:
            $name = 'tRefN';
            $size = 5;
            $data = $this->_readBIFF8CellAddressB(mb_substr($formulaData, 1, 4), $baseCell);
            break;
        ...
        case 0x6D:
            $name = 'tAreaN';
            $size = 9;
            $data = $this->_readBIFF8CellRangeAddressB(mb_substr($formulaData, 1, 8), $baseCell);
            break;
        ...
        case 0x7A:
            ...
            try {
                ...
                $cellAddress = $this->_readBIFF8CellAddress(mb_substr($formulaData, 3, 4));
        ...
        case 0x7B:
            ...
            try {
                ...
                $cellRangeAddress = $this->_readBIFF8CellRangeAddress(mb_substr($formulaData, 3, 8));
    ...
    private function _readBIFF8CellRangeAddressList($subData)
    {
        ...
        for ($i = 0; $i < $nm; ++$i) {
            $cellRangeAddresses[] = $this->_readBIFF8CellRangeAddressFixed(mb_substr($subData, $offset, 8));
            $offset += 8;
        }
    ...
    private function _readBIFF5CellRangeAddressList($subData)
    {
        ...
        for ($i = 0; $i < $nm; ++$i) {
            $cellRangeAddresses[] = $this->_readBIFF5CellRangeAddressFixed(mb_substr($subData, $offset, 6));
            $offset += 6;
        }
        ...
        if (isset($this->_ref[$index])) {
            ...
            switch ($type) {
                case 'internal':
                    ...
                    if (0xFFFF == $this->_ref[$index]['firstSheetIndex'] or 0xFFFF == $this->_ref[$index]['lastSheetIndex']) {
    ...
    private static function _readBIFF8Constant($valueData)
    {
        ...
        case 0x01: // number
            ...
            $value = self::_extractNumber(mb_substr($valueData, 1, 8));
            $size = 9;
            break;
        case 0x02: // string value
            // offset: 1; size: var; Unicode string, 16-bit string length
            $string = self::_readUnicodeStringLong(mb_substr($valueData, 1));
            $value = '"' . $string['value'] . '"';
            $size = 1 + $string['size'];
            break;
        case 0x04: // boolean
    ...
    private function _readByteStringShort($subData)
    {
        ...
        $value = $this->_decodeCodepage(mb_substr($subData, 1, $ln));
    ...
    private function _readByteStringLong($subData)
    {
        ...
        $value = $this->_decodeCodepage(mb_substr($subData, 2));
    ...
    private static function _readUnicodeStringShort($subData)
    {
        ...
        $string = self::_readUnicodeString(mb_substr($subData, 1), $characterCount);
    ...
    private static function _readUnicodeStringLong($subData)
    {
        ...
        $string = self::_readUnicodeString(mb_substr($subData, 2), $characterCount);
    ...
    private static function _readUnicodeString($subData, $characterCount)
    {
        ...
        // needs to be fixed
        $value = self::_encodeUTF16(mb_substr($subData, 1, $isCompressed ? $characterCount : 2 * $characterCount), $isCompressed);
