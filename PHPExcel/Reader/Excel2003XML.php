...
class PHPExcel_Reader_Excel2003XML extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    public function canRead($pFilename)
    {
        ...
        if (preg_match('/<?xml.*encoding=[\'"](.*?)[\'"].*?>/um', $data, $matches)) {
            $this->_charSet = mb_strtoupper($matches[1]);
        }
...
//												$thisBorder['style'] = $borderStyleValue;
                            ...
                            foreach ($styleAttributes as $styleAttributeKey => $styleAttributeValue) {
                                ...
                                switch ($styleAttributeKey) {
                                    ...
                                    case 'Size':
                                            ...
                                            break;
                                    case 'Color':
                                            $this->_styles[$styleID]['font']['color']['rgb'] = mb_substr($styleAttributeValue, 1);
                                            break;
                                    case 'Bold':
                    ...
                    case 'Interior':
                            foreach ($styleAttributes as $styleAttributeKey => $styleAttributeValue) {
                                ...
                                switch ($styleAttributeKey) {
                                    case 'Color':
                                            $this->_styles[$styleID]['fill']['color']['rgb'] = mb_substr($styleAttributeValue, 1);
                                            break;
                                }
                            }
                            break;
...
//			echo '<hr>';
        ...
        foreach ($xml_ss->Worksheet as $worksheet) {
            ...
            if ((isset($this->_loadSheetsOnly)) && (isset($worksheet_ss['Name'])) &&
                (!in_array($worksheet_ss['Name'], $this->_loadSheetsOnly, true))) {
                continue;
            }
