...
//				print_r($sharedFormulas);
    ...
    public function _getFromZipArchive($archive, $fileName = '')
    {
        ...
        if (false === $contents) {
            $contents = $archive->getFromName(mb_substr($fileName, 1));
        }
                ...
                case 'http://schemas.openxmlformats.org/officeDocument/2006/relationships/officeDocument':
                    ...
                    if ($xmlWorkbook->sheets) {
                        foreach ($xmlWorkbook->sheets->sheet as $eleSheet) {
                            ...
                            // Check if sheet should be skipped
                            if (isset($this->_loadSheetsOnly) && !in_array((string) $eleSheet['name'], $this->_loadSheetsOnly, true)) {
                                ++$countSkippedSheets;
                            ...
                            if (!$this->_readDataOnly && $xmlSheet && $xmlSheet->conditionalFormatting) {
                                foreach ($xmlSheet->conditionalFormatting as $conditional) {
                                    foreach ($conditional->cfRule as $cfRule) {
                                        if (
                                            (
                                                PHPExcel_Style_Conditional::CONDITION_NONE == (string)$cfRule['type'] ||
                                                PHPExcel_Style_Conditional::CONDITION_CELLIS == (string)$cfRule['type'] ||
                                                PHPExcel_Style_Conditional::CONDITION_CONTAINSTEXT == (string)$cfRule['type'] ||
                                                PHPExcel_Style_Conditional::CONDITION_EXPRESSION == (string)$cfRule['type']
