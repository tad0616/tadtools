    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
    {
        ...
        while (false !== ($rowData = fgets($fileHandle))) {
            ...
            if ('P' == $dataType) {
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        case 'P':	$formatArray['numberformat']['code'] = str_replace($fromFormats, $toFormats, mb_substr($rowDatum, 1));
                                    break;
                        case 'E':
                        case 'F':	$formatArray['font']['name'] = mb_substr($rowDatum, 1);
                                    break;
                        case 'L':	$formatArray['font']['size'] = mb_substr($rowDatum, 1);
                                    break;
                        case 'S':	$styleSettings = mb_substr($rowDatum, 1);
            ...
            //	Read cell value data
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        case 'C':
                        case 'X':	$column = mb_substr($rowDatum, 1);
                                    break;
                        case 'R':
                        case 'Y':	$row = mb_substr($rowDatum, 1);
                                    break;
                        case 'K':	$cellData = mb_substr($rowDatum, 1);
                                    break;
                        case 'E':	$cellDataFormula = '=' . mb_substr($rowDatum, 1);
                ...
                foreach ($rowData as $rowDatum) {
                    switch ($rowDatum[0]) {
                        ...
                        case 'P':	$formatStyle = $rowDatum;
                                    break;
                        case 'W':	list($startCol, $endCol, $columnWidth) = explode(' ', mb_substr($rowDatum, 1));
                                    break;
                        case 'S':	$styleSettings = mb_substr($rowDatum, 1);
                                    for ($i = 0; $i < mb_strlen($styleSettings); ++$i) {
                                        switch ($styleSettings[$i]) {
