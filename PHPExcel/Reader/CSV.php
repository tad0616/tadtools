...
class PHPExcel_Reader_CSV extends PHPExcel_Reader_Abstract implements PHPExcel_Reader_IReader
{
    ...
    protected function _skipBOM()
    {
        ...
        switch ($this->_inputEncoding) {
            case 'UTF-8':
                "\xEF\xBB\xBF" == fgets($this->_fileHandle, 4) ?
                    fseek($this->_fileHandle, 3) : fseek($this->_fileHandle, 0);
                break;
            case 'UTF-16LE':
                "\xFF\xFE" == fgets($this->_fileHandle, 3) ?
                    fseek($this->_fileHandle, 2) : fseek($this->_fileHandle, 0);
                break;
            case 'UTF-16BE':
                "\xFE\xFF" == fgets($this->_fileHandle, 3) ?
                    fseek($this->_fileHandle, 2) : fseek($this->_fileHandle, 0);
                break;
            case 'UTF-32LE':
                "\xFF\xFE\x00\x00" == fgets($this->_fileHandle, 5) ?
                    fseek($this->_fileHandle, 4) : fseek($this->_fileHandle, 0);
                break;
            case 'UTF-32BE':
                "\x00\x00\xFE\xFF" == fgets($this->_fileHandle, 5) ?
                    fseek($this->_fileHandle, 4) : fseek($this->_fileHandle, 0);
                break;
            default:
                break;
    ...
    public function listWorksheetInfo($pFilename)
    {
        ...
        while (false !== ($rowData = fgetcsv($fileHandle, 0, $this->_delimiter, $this->_enclosure))) {
    ...
    public function loadIntoExisting($pFilename, PHPExcel $objPHPExcel)
    {
        ...
        $currentRow = 1;
        if ($this->_contiguous) {
            $currentRow = (-1 == $this->_contiguousRow) ? $sheet->getHighestRow() : $this->_contiguousRow;
        }
