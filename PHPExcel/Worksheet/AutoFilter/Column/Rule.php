...
class PHPExcel_Worksheet_AutoFilter_Column_Rule
{
    ...
    {
        if (!in_array($pRuleType, self::$_ruleTypes, true)) {
    ...
    public function setValue($pValue = '')
    {
        if (is_array($pValue)) {
            ...
            foreach ($pValue as $key => $value) {
                ...
                if (!in_array($key, self::$_dateTimeGroups, true)) {
                    //	Remove any invalid entries from the value array
                    unset($pValue[$key]);
                } else {
                    //	Work out what the dateTime grouping will be
                    $grouping = max($grouping, array_search($key, self::$_dateTimeGroups, true));
                }
            }
            if (0 == count($pValue)) {
    ...
    public function setOperator($pOperator = self::AUTOFILTER_COLUMN_RULE_EQUAL)
    {
        ...
        if ((!in_array($pOperator, self::$_operators, true)) &&
            (!in_array($pOperator, self::$_topTenValue, true))) {
    ...
    {
        if ((null !== $pGrouping) &&
            (!in_array($pGrouping, self::$_dateTimeGroups, true)) &&
            (!in_array($pGrouping, self::$_dynamicTypes, true)) &&
            (!in_array($pGrouping, self::$_topTenType, true))) {
            throw new PHPExcel_Exception('Invalid rule type for column AutoFilter Rule.');
        }
