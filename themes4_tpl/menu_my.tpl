<{foreach from=$menu_var item=m1}>
    <li class="nav-item <{if $m1.submenu|default:false}>dropdown<{/if}>">
        <a class="nav-link <{if $m1.submenu|default:false}>dropdown-toggle<{/if}>" <{if $m1.url!=''}>href="<{if $m1.target=="popup"}>javascript:tad_themes_popup('<{$m1.url}>');<{else}><{$m1.url}><{/if}>" <{if $m1.target!="popup"}>target="<{$m1.target}>" <{if $m1.target=='_blank'}>title="開新視窗並連至<{$m1.title}>"<{/if}><{/if}><{/if}>><{if $m1.img|default:false}><img src="<{$m1.img}>" alt="<{$m1.title}> icon"><{elseif $m1.icon}><i class="<{if $m1.icon|substr:0:3=='fa-'}>fa <{/if}><{$m1.icon}>"></i><{/if}> <{$m1.title}></a>
