...
class TadUpFiles
{
    ...
    public function __construct($prefix = '', $subdir = '', $file = '/file', $image = '/image', $thumbs = '/image/.thumbs')
    {
        ...
        $this->mime_type_check = $TadToolsModuleConfig['mime_type_check'];
    }
    ...
    //設定縮圖背景
    public function set_thumb($width = '', $height = '', $bg_color = '', $position = '', $repeat = '', $size = ''): void
    {
        if (!empty($width)) {
    ...
    //設定目錄
    public function set_dir($type, $dir = ''): void
    {
        if ($type === 'subdir') {
            $this->subdir = $dir;
        } elseif ($type === 'file') {
    ...
    public function set_var($name = '', $val = ''): void
    {
        $this->$name = $val;
    }

    public function set_col($col_name = '', $col_sn = '', $sort = ''): void
    {
    ...
    public function set_files_sn($files_sn = ''): void
    {
        $this->files_sn = $files_sn;
    }

    //是否套用fancybox
    public function set_fancybox($show = true): void
    {
        $this->showFancyBox = $show;
    }

    //自己設定檔案下載路徑
    public function set_download_url($url = ''): void
    {
        $this->download_url = $url;
    }

    //是否加密
    public function set_hash($hash = false): void
    {
        $this->hash = $hash;
    }
    ...
    private function get_groups()
    {
        ...
        $permission = _TUF_PERMISSION_NOTE;
        $groups = [];
        while ([$groupid, $name] = $xoopsDB->fetchRow($result)) {
            $groups[$groupid] = $name;
        }
        ...
        while ($all = $xoopsDB->fetchArray($result)) {
            ...
            if ($show_edit === true or $show_edit === 'full') {
                ...
                if ($this->permission) {
                    ...
                    $result = $xoopsDB->query($sql) or web_error($sql);
                    $gperm_groupid_arr = [];
                    while ([$gperm_groupid] = $xoopsDB->fetchRow($result)) {
    ...
    //做縮圖
    protected function thumbnail($filename = '', $thumb_name = '', $type = 'image/jpeg', $width = '120'): void
    {
    ...
    public function auto_sort()
    {
        ...
        $result = $xoopsDB->queryF($sql) or web_error($sql, __FILE__, __LINE__);
        [$max] = $xoopsDB->fetchRow($result);

        return ++$max;
    }

    //更新某個欄位值
    protected function update_col_val($files_sn = '', $col = '', $val = ''): void
    {
        global $xoopsDB, $xoopsUser;
    ...
    //改檔名
    public function rename_file($files_sn = '', $new_name = ''): void
    {
        if (empty($files_sn)) {
            return;
        }
    ...
    public function get_file_amount()
    {
        ...
        $result = $xoopsDB->queryF($sql) or web_error($sql, __FILE__, __LINE__);
        [$amount] = $xoopsDB->fetchRow($result);
    ...
    //下載並新增計數器
    public function add_file_counter($files_sn = '', $hash = false, $force = false, $path = ''): void
    {
        global $xoopsDB, $xoopsUser;
