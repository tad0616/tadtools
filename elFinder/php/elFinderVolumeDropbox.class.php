...
class elFinderVolumeDropbox extends elFinderVolumeDriver
{
        ...
        if ('init' === $options['user']) {
            ...
            if ('init' === $options['pass']) {
                ...
                if (!$html) {
                    ...
                    if (0 !== mb_strpos($options['url'], 'http')) {
    ...
    public function netunmount($netVolumes, $key)
    {
        ...
        foreach ($netVolumes as $volume) {
            if ('dropbox' === $volume['host'] && $volume['dropboxUid'] === $dropboxUid) {
                $count++;
            }
        }
        if (1 === $count) {
    ...
    private function checkDB()
    {
        ...
        if ($res && isset($_REQUEST['init'])) {
            ...
            if (!$chk || false === mb_strpos(mb_strtoupper($chk[0]), 'UNIQUE')) {
                // remake
                $this->DB->exec('DROP TABLE ' . $this->DB_TableName);
                $res = false;
            }
    ...
    protected function deltaCheck($refresh = true)
    {
        ...
        if ($chk && $_SERVER['REQUEST_TIME'] < ($chk['mtime'] + $this->options['metaCacheTime'])) {
            return true;
        }
    ...
    protected function parseRaw($raw)
    {
        ...
        $stat['size'] = 'directory' == $stat['mime'] ? 0 : $raw['bytes'];
    ...
    protected function doSearch($path, $q, $mimes)
    {
        ...
        $sth->execute([(('/' === $path) ? '' : mb_strtolower($path)) . '%', '%' . mb_strtolower($q) . '%']);
        $res = $sth->fetchAll(PDO::FETCH_COLUMN);
        ...
        if ($res) {
            foreach ($res as $raw) {
                ...
                if ($stat = $this->parseRaw($raw)) {
                    ...
                    if (!empty($stat['hidden']) || ($mimes && 'directory' === $stat['mime']) || !$this->mimeAccepted($stat['mime'], $mimes)) {
                        continue;
                    }
    ...
    public function getContentUrl($hash, $options = [])
    {
        if (false === ($file = $this->file($hash)) || !$file['url'] || 1 == $file['url']) {
            ...
            $url = '';
            if (isset($cache['share']) && false !== mb_strpos($cache['share'], $this->dropbox_dlhost)) {
            ...
            if (!$url) {
                try {
                    ...
                    $url = $res['url'];
                    if (false === mb_strpos($url, 'www.dropbox.com')) {
    ...
    {
        return $this->_normpath(mb_substr($path, 0, mb_strrpos($path, '/')));
    }
    ...
    protected function _fopen($path, $mode = 'rb')
    {
        ...
        if ($this->tmp) {
            ...
            if ($local = $this->getTempFile($path)) {
                if (false !== file_put_contents($local, $contents, LOCK_EX)) {
