...
class elFinder
{
    ...
    protected $commands = [
        ...
        'upload' => ['target' => true, 'FILES' => true, 'mimes' => false, 'html' => false, 'upload' => false, 'name' => false, 'upload_path' => false, 'chunk' => false, 'cid' => false, 'node' => false, 'renames' => false, 'hashes' => false, 'suffix' => false, 'mtime' => false],
    ...
    public function __construct($opts)
    {
        ...
        if (!interface_exists('elFinderSessionInterface')) {
            require_once __DIR__ . '/elFinderSessionInterface.php';
        }
        ...
        if (!empty($opts['session']) && $opts['session'] instanceof elFinderSessionInterface) {
            ...
            if (!class_exists('elFinderSession')) {
                require_once __DIR__ . '/elFinderSession.php';
            }
        ...
        if (!empty($opts['bind']) && is_array($opts['bind'])) {
            ...
            foreach ($opts['bind'] as $cmd => $handlers) {
                ...
                if (!$doRegist) {
                    ...
                    $doRegist = ($_reqCmd && in_array($_reqCmd, array_map($_getcmd, explode(' ', $cmd)), true));
                }
                if ($doRegist) {
    ...
    protected function parents($args)
    {
        ...
        if (false === ($volume = $this->volume($target))
        || false === ($tree = $volume->parents($target))) {
            return ['error' => $this->error(self::ERROR_OPEN, '#' . $target)];
        }
    ...
    protected function zipdl($args)
    {
        ...
        // 2nd: Return array contains opened file pointer, root itself and required headers
        if (4 !== count($targets) || false === ($volume = $this->volume($targets[0]))) {
    ...
    protected function mkdir($args)
    {
        ...
        if ($dirs) {
            ...
            return false === ($res = $this->ensureDirsRecursively($volume, $target, $mkdirs))
    ...
    protected function parse_data_scheme($str, $extTable)
    {
        ...
        if ($fp = fopen('data://' . mb_substr($str, 5), 'rb')) {
            if ($data = stream_get_contents($fp)) {
                ...
                $name = mb_substr(md5($data), 0, 8) . $ext;
            }
            fclose($fp);
        }
    ...
    protected function detectFileExtension($path)
    {
        ...
        if ($mime) {
            ...
            if (in_array($mime, ['application/x-empty', 'inode/x-empty'], true)) {
    ...
    protected function upload($args)
    {
        ...
        foreach ($files['name'] as $i => $name) {
            if (($error = $files['error'][$i]) > 0) {
                $result['warning'] = $this->error(self::ERROR_UPLOAD_FILE, $name, UPLOAD_ERR_INI_SIZE == $error || UPLOAD_ERR_FORM_SIZE == $error ? self::ERROR_UPLOAD_FILE_SIZE : self::ERROR_UPLOAD_TRANSFER);
            ...
            if ('blob' === $name) {
                ...
                $name = mb_substr(md5(basename($tmpname)), 0, 8) . $ext;
            }
            ...
            if (!$_target || false === ($file = $volume->upload($fp, $_target, $name, $tmpname, $hashes))) {
    ...
    protected function get($args)
    {
        ...
        if ($mime && 'text' === mb_strtolower(mb_substr($mime, 0, 4))) {
            $enc = '';
    ...
    {
        if (self::$base64encodeSessionData) {
            $data = unserialize(base64_decode($var, true));
        } else {
            $data = $var;
        }
        $chk = true;
