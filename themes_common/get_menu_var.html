<{php}>
  ...
  if(!empty($mod_menu)){
    if(empty($my_menu))$my_menu=array();
  ...
  function get_theme_menu_items($id="", $other_menu=true){
    ...
    if(strpos($_SESSION['menu_var_kind'],'all')!==false or strpos($_SESSION['menu_var_kind'],'my_menu')!==false){
      $sql="select `menuid`, `itemname`, `itemurl`, `target`, `icon`, `link_cate_name`, `link_cate_sn` from ".$xoopsDB->prefix("tad_themes_menu")." where of_level='{$id}' and status='1' order by position";
      $result = $xoopsDB->query($sql);

      $modhandler      = xoops_gethandler('module');
      if($result){
        $i=0;
        while(list($menuid,$itemname,$itemurl,$target,$icon,$link_cate_name,$link_cate_sn)=$xoopsDB->fetchRow($result)){
