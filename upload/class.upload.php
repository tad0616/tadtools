...
class upload
{
     ...
     */
    public function upload($file, $lang = 'en_TW')
    {
        $this->version = '0.32';
                ...
                if ($this->processed && !($f = @fopen($this->file_dst_path . $hash . (!empty($this->file_dst_name_ext) ? '.' . $this->file_dst_name_ext : ''), 'a+'))) {
                    if ($this->dir_auto_chmod) {
                        ...
                        if (!@chmod($this->file_dst_path, $this->dir_chmod)) {
                            $this->log .= ' failed<br />';
                            $this->processed = false;
                            $this->error     = $this->translate('destination_dir_write', array($this->file_dst_path));
                        } else {
                            $this->log .= ' success<br />';
                        ...
                        $this->processed = false;
                        $this->error     = $this->translate('destination_path_write', array($this->file_dst_path));
                    }
                } else {
                    if ($this->processed) {
                        @fclose($f);
