<{foreach from=$menu_var item=m}>
    <li>
        <a <{if $m.submenu|default:false}>class="dropdown-toggle" data-toggle="dropdown" <{/if}> <{if $m.url!=''}>href="<{if $m.target=="popup"}>javascript:tad_themes_popup('<{$m.url}>');<{else}><{$m.url}><{/if}>" <{if $m.target!="popup"}>target="<{$m.target}>" <{if $m.target=='_blank'}>title="開新視窗並連至<{$m.title}>"<{/if}><{/if}><{/if}>>
