...
<{if $user_ownpage == true}>
    ...
    <form name="usernav" action="user.php" method="post">
        <input class="btn btn-success btn-sm" type="button" value="<{$lang_editprofile|default:''}>" onclick="location='<{$xoops_url}>/modules/<{$xoops_dirname|default:''}>/edituser.php'">
        <input class="btn btn-success btn-sm" type="button" value="<{$lang_changepassword|default:''}>" onclick="location='<{$xoops_url}>/modules/<{$xoops_dirname|default:''}>/changepass.php'">
        <{if $user_changeemail|default:false}>
		<input class="btn btn-success btn-sm" type="button" value="<{$smarty.const._PROFILE_MA_CHANGEMAIL}>" onclick="location='<{$xoops_url}>/modules/<{$xoops_dirname|default:''}>/changemail.php'">
        <{/if}>
        ...
        <form method="post" action="<{$xoops_url}>/modules/<{$xoops_dirname|default:''}>/user.php">
            ...
            <input class="btn btn-success btn-sm" type="button" value="<{$lang_deleteaccount|default:''}>" onclick="submit();">
        </form>
        <{/if}>

        <input class="btn btn-success btn-sm" type="button" value="<{$lang_avatar|default:''}>" onclick="location='edituser.php?op=avatarform'">
        <input class="btn btn-success btn-sm" type="button" value="<{$lang_inbox|default:''}>" onclick="location='<{$xoops_url}>/viewpmsg.php'">
        <input class="btn btn-success btn-sm" type="button" value="<{$lang_logout|default:''}>" onclick="location='<{$xoops_url}>/modules/<{$xoops_dirname|default:''}>/user.php?op=logout'">
    </form>

<{elseif $xoops_isadmin != false}>
    <form method="post" action="<{$xoops_url}>/modules/<{$xoops_dirname|default:''}>/admin/deactivate.php">
        <input class="btn btn-info btn-sm" type="button" value="<{$lang_editprofile|default:''}>" onclick="location='<{$xoops_url}>/modules/<{$xoops_dirname|default:''}>/admin/user.php?op=edit&amp;id=<{$user_uid|default:''}>'">
        <input type="hidden" name="uid" value="<{$user_uid|default:''}>">
        <{if $userlevel == 1}>
