<{foreach from=$menu_var item=m1}>
    <li role="none">
        ...
        <{else}>
            <a href="<{if $m1.url!=''}><{if $m1.target=='popup'}>javascript:tad_themes_popup('<{$m1.url}>');<{else}><{$m1.url}><{/if}><{else}>#<{/if}>" role="menuitem" <{if $m1.url!='' && $m1.target!='popup'}>target="<{$m1.target}>" <{if $m1.target=='_blank'}>title="開新視窗並連至<{$m1.title}>"<{/if}><{/if}>>
